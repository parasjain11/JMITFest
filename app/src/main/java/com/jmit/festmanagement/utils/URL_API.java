package com.jmit.festmanagement.utils;

/**
 * Created by arpitkh996 on 07-09-2016.
 */

public class URL_API {
    public static final String main = "http://jmit_fests.net16.net";
    public static final String baseUrl = main;
    public static final String LOGIN_API = baseUrl + "/user_login.php";
    public static final String ADMIN_LOGIN=baseUrl+"/admin_login.php";
    public static final String FESTS=baseUrl+"/get_fests.php";

    public static final String EDIT_DETAILS=baseUrl+"/edit_details.php";

    public static final String UNREGISTRATION=baseUrl+"/unregister.php";
    public static final String EVENTS=baseUrl+"/get_events.php";
    public static final String REGISTRATION=baseUrl+"/registeration.php";
    public static final String SIGN_UP=baseUrl+"/create_user.php";
    public static final String REGISTERED_EVENTS=baseUrl+"/get_registered_events.php";

    public static final String ADMIN_REGISTERED_EVENTS=baseUrl+"/get_registeration.php";
}
